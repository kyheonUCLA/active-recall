...
const GenerateButton: FC = () => {
  const { setResponse, user } = useAllContext();
  const fetchData = async () => {
    const prompt = `greet ${user.firstname} ${user.lastname}`;
    try {
  ...
  return (
    <button onClick={fetchData} className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4
